const creds={user:"tidewatcher",pass:{pl:"MorzePamietaWszystko!1987",en:"TheSeaRemembersEverything!1987"}};
...
$('#loginForm').addEventListener('submit',e=>{e.preventDefault();const u=$('#username').value.trim().toLowerCase(),p=$('#password').value,m=$('#loginMessage');if(!u||!p){m.textContent=lang==='en'?'Enter username and password.':'Uzupełnij nazwę użytkownika i hasło.';return}if(u!==creds.user){m.textContent=lang==='en'?'User not found.':'Nie znaleziono użytkownika o podanej nazwie.';return}const expectedPassword =
lang === "en"
? creds.pass.en
: creds.pass.pl;
if(p!==expectedPassword){m.textContent=lang==='en'?'Incorrect password. Check the spelling and try again.':'Nieprawidłowe hasło. Sprawdź zapis i spróbuj ponownie.';return}m.style.color='#39745b';m.textContent=lang==='en'?'Authentication complete. Opening archive…':'Uwierzytelnianie zakończone. Otwieranie archiwum…';localStorage.setItem('tidearchive_authenticated','true');setTimeout(()=>show('#archiveView'),450)});
